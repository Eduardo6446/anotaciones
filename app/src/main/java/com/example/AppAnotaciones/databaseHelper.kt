...
class databaseHelper(context: Context): SQLiteOpenHelper(context,
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val createTable = "CREATE TABLE ${constants.ENTITY_TAREA} (" +
                "${constants.PROPERTY_ID} INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "${constants.PROPERTY_DESCRIPCION} VARCHAR(120), " +
                "${constants.PROPERTY_FINALIZADO} BOOLEAN)"
    ...
    @SuppressLint("Range")
    fun getTarea():MutableList<Anotacion>{
